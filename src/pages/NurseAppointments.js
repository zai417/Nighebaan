...
function ConfirmModal({ appt, onClose, onConfirmed }) {
  ...
  return (
    <div onClick={onClose} className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 overflow-auto">
      <div onClick={(e) => e.stopPropagation()} className="w-full max-w-md rounded-3xl bg-white shadow-2xl ring-1 ring-slate-200 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between border-b border-slate-100 px-7 py-5">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.3em] text-teal-600">Accept Appointment</p>
...
export default function NurseAppointments() {
  ...
  const loadAppointments = useCallback((n) => {
    const all = JSON.parse(localStorage.getItem(STORAGE_KEY) || "[]");
    const mine = all.filter((a) => a.providerType === 'nurse' && a.providerId === (n?.id || n?._id));
  ...
  const handleConfirmed = () => {
    ...
    setSuccessMsg("Appointment accepted and patient has been notified.");
    setTimeout(() => setSuccessMsg(""), 4000);
    loadAppointments(nurse);
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <main className="mx-auto max-w-4xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        <p className="mt-1 text-sm text-slate-500 mb-8">Review and accept patient appointment requests.</p>
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        ...
                        <div className={`mt-3 rounded-xl border px-3 py-2.5 text-sm ${
                          ...
                          <p className="text-xs">
                            PKR {(appt.paymentAmount || 1500).toLocaleString()}
